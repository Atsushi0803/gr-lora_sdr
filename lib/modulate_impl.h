...
class modulate_impl : public modulate
{
   private:
    ...
    uint32_t
        m_samples_per_symbol;  ///< samples per symbols(Works only for 2^sf)
    ...
    uint32_t
        preamb_symb_cnt;  ///< counter of the number of preamble symbols output
    uint32_t padd_cnt;   ///< counter of the number of null symbols output after
                         ///< each frame
