...
namespace lora_sdr
{
        ...
        gr_vector_void_star& output_items)
    {
        ...
        if (tags.size()) {
            ...
            if (tags[0].offset != nitems_read(0))
                ...
                if (tag_length >= 30) {
                    ...
                    // tags[0].value = pmt::from_long(int((m_frame_len +
                    ...
                    tags[0].value = pmt::from_long(
                        int((apcma_code_length + 20) * m_samples_per_symbol));
        ...
        for (int i = 0; i < m_npulse; i++) {
            int tmp_index = std::accumulate(l_interval.begin(),
                                std::next(l_interval.begin(), i), 0)
                + i; // パルスがオンになるm_codewordのインデックスを作成
        ...
        {
            for (int i = 0;
                 i < (noutput_items - output_offset) / m_samples_per_symbol; i++) {
                if (padd_cnt < m_inter_frame_padding) {
                    for (int j = 0; j < m_samples_per_symbol; j++) {
                        out[output_offset + j] = gr_complex(0.0, 0.0);
                    }
